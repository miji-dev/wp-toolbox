	...
	public function fields(): array {
		return [
			Field::choice(
				...
				__('Shows a coloured label in the toolbar: "Live" (red), "Staging" (yellow), "Development" (blue) or "Local" (green).', 'wptb'),
			...
			Field::bool(
				...
				__('On staging, development and local sites, every page tells search engines not to index it or follow its links, regardless of the "Search engine visibility" setting. The live site is not affected.', 'wptb'),
			...
			Field::choice(
				...
				__('What happens to emails sent by staging, development and local sites (form notifications, orders, password resets, …). Redirected emails show the original recipients in the subject. The live site is not affected.', 'wptb'),
				why: __('A staging copy contains real customers and form recipients. Without this, testing an order form or a newsletter on staging sends real emails to real people.', 'wptb'),
				sideEffects: __('Blocked emails are reported as sent, so forms and shops don\'t show errors. Mail plugins that don\'t use WordPress\' mail function are not affected.', 'wptb'),
			),
			Field::text(
				'mail_redirect_to',
				'',
				__('Redirect emails to', 'wptb'),
				__('Email address that receives all emails when "Send all emails to the address below" is chosen. If it isn\'t a valid address, no emails are sent at all.', 'wptb'),
